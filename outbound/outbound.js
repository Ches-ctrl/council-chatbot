...
async function getSignedUrl() {
  try {
    console.log(`[ElevenLabs] Requesting signed URL for agent ID: ${ELEVENLABS_AGENT_ID}`);
                ...
                case "audio":
                  if (streamSid) {
                    if (message.audio?.chunk) {
                      // Make sure the audio chunk is properly base64 encoded
                      const audioChunk = typeof message.audio.chunk === 'string'
                        ? message.audio.chunk
                        : Buffer.from(message.audio.chunk).toString('base64');
          ...
          switch (msg.event) {
            ...
            case "media":
              if (elevenLabsWs?.readyState === WebSocket.OPEN) {
                // Don't double-encode the audio data
                const audioMessage = {
